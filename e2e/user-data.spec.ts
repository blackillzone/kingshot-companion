...
test.describe("User Data Section E2E", () => {
  ...
  test("should display tabs or navigation for user data sections", async ({ page }) => {
    // Look for tabs (Heroes, Governor, Troops, Static Stats)
    const tabs = page.locator("button[role=tab], [role=tablist] button, [class*=tab]");
  ...
  test("should have form inputs for user data editing", async ({ page }) => {
    ...
    if (count > 0) {
      await expect(inputs.first()).toBeVisible().catch(() => {
